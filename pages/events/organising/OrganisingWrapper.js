...
function organisingWrapper({ newsData, incrementPagination, type }) {
  //const { newsItems = [], moreNewsExist } = newsData;
  if (!newsData) {
    return false;
  }
  const { organisingWrapper: newsItems = [], moreNewsExist } = newsData;
        ...
        <Row>
          {newsItems.map((events, i) => (
            <Col key={events.id} md={6} lg={4}>
              <div className="news-box  wow fadeInUp" data-wow-delay={`.${i}s`}>
                ...
                <Link href={`organising/${events.alias}`} className="btns_line">
                  KNOW MORE
                </Link>
              </div>
            </Col>
...
export default organisingWrapper;
