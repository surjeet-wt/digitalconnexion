...
import OrganisingWrapper from "./OrganisingWrapper";
...
export async function getServerSideProps({ query }) {
  try {
    ...
    const axiosConfig = {
      method: "get",
      url: `/events/organising?page=1&type=${type || "all"}`,
      headers: {
...
<CustomHead {...pageData} />
      ...
      {newsData && !hasErr && type === "all" || type === "past" || type === "upcoming" ? (
        <OrganisingWrapper newsData={newsData} incrementPagination={incrementPagination} type={type} />
      )  : null}
      <Footer />
    </>
  );
